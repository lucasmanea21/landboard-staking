...
const iconsPath = path.join(process.cwd(), process.env.ICONS_FOLDER || "../src/assets/icons/");
...
const indexContent = [
	...
	icons
		.map(
			(icon) =>
				`export const ${toPascalCase(icon)}: React.FC<IconProps> = (props) => <Icon {...props} name="${icon}" />;`
		)
		.join("\n"),
].join("\n");
...
const iconListContent = ["export default [", icons.map((icon) => `"${icon}"`), "];"].join("");
